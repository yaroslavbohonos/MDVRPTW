...
class TestGA(unittest.TestCase):
    ...
    def testFeasibilityInvalidTW(self):
        ...
        customers[0] = (Customer(1, 10, 10, 3, 19, 20))
        customers[1] = (Customer(2, 40, 10, 3, 8, 12))
        customers[2] = (Customer(3, 40, 40, 3, 14, 15))
        
        routes = [[depots[0], customers[0], customers[1], customers[2], depots[0] ]]
        solution.routes = routes
        solution.isFeasible = GA.isFeasible(solution)
        self.assertFalse(solution.isFeasible)
        
        routes = [[depots[0], customers[2], customers[0], customers[1], depots[0] ]]
        solution.routes = routes
        solution.isFeasible = GA.isFeasible(solution)
        self.assertFalse(solution.isFeasible)

        routes = [[depots[0], customers[1], customers[2], customers[0], depots[0] ]]
        solution.routes = routes
        solution.isFeasible = GA.isFeasible(solution)
        self.assertTrue(solution.isFeasible)
    ...
    def testMakeFeasible(self):
        ...
        depots.append(Depot(2, 80, 80, 40, 8, 20))
        customers = []
        customers.append(Customer(1, 10, 10, 3, 9, 12))
        customers.append(Customer(2, 40, 10, 3, 10, 11))
        customers.append(Customer(3, 40, 40, 3, 8, 9))
        customers.append(Customer(4, 90, 90, 3, 8, 9))
        customers.append(Customer(5, 90, 30, 3, 10, 12))
        ...
        routes = [[depots[0], customers[0], customers[1], customers[4], depots[0] ],
                  [depots[1], customers[2], customers[3], customers[4], customers[5], depots[1]] ]
        solution = Solution(routes, None, None)
        GA.calculateDistMatrix()
        GA.calculateTimeMatrix()
        GA.makeFeasible()
